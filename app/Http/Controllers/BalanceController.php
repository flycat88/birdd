...
class BalanceController extends Controller
{
    ...
    // Constructor to inject SMSService
    ...
    public function calculateBalances()
    {
        try {
            ...
            foreach ($tenants as $tenant) {
                ...
                // Optionally send SMS to the tenant
                $message = $this->formatBalanceMessage($tenant, $balance);
                $this->smsService->sendSMS($tenant->phone_number, $message);
            }
            ...
            return redirect()->back()
                ->withErrors(['error' => 'Error calculating balances: ' . $e->getMessage()]);
        }
    }

    // Format balance message for SMS
    private function formatBalanceMessage($tenant, $balance)
    {
        $message = "Dear {$tenant->name},\n";
        $message .= "House: {$tenant->house_number}\n";

        if ($balance > 0) {
            $message .= "Outstanding Balance: KES " . number_format($balance, 2);
        } elseif ($balance < 0) {
            $message .= "Credit Balance: KES " . number_format(abs($balance), 2);
        } else {
            $message .= "Your account is fully settled.";
        }
